...
TMPDIR='/Users/skippern/Documents/Maps/tmp2/'
BSBDIR='/Users/skippern/Documents/Maps/BSB/'
TILEDIR='/Users/skippern/Documents/Maps/RNC'
...
for i in `cat ../Process.txt`; do scale=`head -n 15 $i | grep KNP | sed '/KNP\/SC=/s///' | sed '/,/s// /' | grep -o "^[0-9]*"`
	echo "1:$scale"
	area=`echo $i | sed '/\/Users\/skippern\/Documents\/Maps\/BSB\//s///' | tr '/' '\n' |grep -v KAP`
...
#				mv topW.png top.png 2>/dev/null
				# Merge tiles
				if [ ! -d $TILEDIR/$area/$k ]; then
					mkdir $TILEDIR/$area/$k
				fi
				if [ -e $TILEDIR/$area/$l ]; then
					ret="."
...
#					curl -# -G -f http://a.tiles.openstreetmap.org/$l -o "bottom.png" 2>/dev/null
					dl=`expr $dl + 1`
				fi
